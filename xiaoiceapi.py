        ...
        while times <=8:
            ...
            if text:
                if text.text == input_strs:
                    time.sleep(0.5)
                    continue
            if "收起" in soup.get_text():
                #返回imgURL
                imgUrl = soup.find(href=re.compile('msget')).get('href')
                contend.append(self.dicts(200,imgUrl,"img"))
                '''
                text = "图片地址： " + imgUrl
            ...
            if "mp3" in soup.get_text():
                mp3Url = soup.find(href=re.compile('msget')).get('href')
                contend.append(self.dicts(200, mp3Url,"voice"))
                ...
                '''
            if text is not None:
                contend.append(self.dicts(200, text.text,"text"))
            time.sleep(0.5)
        seen = set()
        new_l = []
        for d in contend:
